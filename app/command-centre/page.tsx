        ...
        <div className="px-6 pt-5 pb-4">
          ...
          <div className="mb-5">
            ...
            <textarea
              autoFocus
              rows={4}
              className="w-full p-3 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#2563EB] resize-none text-slate-800 placeholder:text-slate-400 transition"
...
export default function CommandCentrePage() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 h-full overflow-y-auto overflow-x-hidden w-full bg-[#F8FAFC] dark:bg-[#0a0a0a] text-slate-900 dark:text-slate-100 flex flex-col">
      <div className="max-w-[1600px] mx-auto w-full flex-grow flex flex-col h-full space-y-6">

        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Command Centre</h1>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-64"
            />
          </div>
        </div>
          ...
          <DragDropContext onDragEnd={onDragEnd}>
            <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-6 h-full min-h-[500px]">
              {columns.map(col => {
                ...
                return (
                  <div key={col.status} className="bg-[#F1F5F9] dark:bg-slate-900/50 rounded-2xl flex flex-col h-full overflow-hidden">

                    {/* Column Header */}
                    <div className={`bg-white dark:bg-slate-800 mx-2 mt-2 p-3 flex items-center justify-between rounded-xl shadow-sm border-l-4 ${col.color}`}>
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${col.dot}`} />
                        <h3 className="font-bold text-slate-700 dark:text-slate-200 text-sm">{col.title}</h3>
                      </div>
                      <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-900 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700">
                    ...
                    {col.status === 'In Progress' && (
                      <div className="mx-2 mt-1 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                      ...
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`p-2 mt-1 flex-grow overflow-y-auto space-y-3 transition-all duration-200 rounded-b-2xl ${
                            snapshot.isDraggingOver
                              ? col.status === 'In Progress'
                                ? 'bg-blue-100 dark:bg-blue-900/30 ring-2 ring-inset ring-blue-300'
                        ...
                        >
                            ...
                            <Draggable key={complaint.id} draggableId={complaint.id} index={index}>
                              {(provided, snapshot) => (
                                <div
                                  ...
                                  className={`bg-white dark:bg-slate-800 p-4 rounded-xl border shadow-sm transition-all duration-150 mx-1 select-none ${
                                ...
                                >
                                  ...
                                  {/* Summary */}
                                  <h4 className="text-[13px] font-bold text-slate-800 dark:text-slate-200 mb-2.5 leading-snug">
                                    {complaint.summary}
                                  </h4>
                                  ...
                                  <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium">
                                    ...
                                    {complaint.category && (
                                      <span className="bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-[10px] text-slate-500 truncate">
                                        {complaint.category}
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  {/* Notes snippet */}
                                  {complaint.notes && (
                                    <div className="mt-2.5 pt-2.5 border-t border-slate-100 dark:border-slate-700 flex items-start gap-1.5 text-xs text-slate-500">
                          ...
                          {colComplaints.length === 0 && !snapshot.isDraggingOver && (
                            <div className="h-28 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 text-xs mx-1 gap-1">
