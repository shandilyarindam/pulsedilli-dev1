...
export async function POST(req: NextRequest) {
  try {
    ...
    const { parameters, dateRange } = await req.json();
    if (!parameters || !Array.isArray(parameters)) {
      return NextResponse.json({ success: false, error: 'Parameters array is required.' }, { status: 400 });
    }
...
You need to generate a highly customized text report based on the following selected parameters:
[${parameters.join(', ')}]

The requested date range context is: ${dateRange}

Here is a sample of recent complaints data in JSON format:
${JSON.stringify(minimizedData)}

Based strictly on this data, write a comprehensive 3-5 paragraph report focusing ONLY on the selected parameters. 
If a parameter asks for 'Department Performance', analyze departments. If it asks for 'Category Wise Analysis', break down the categories.
Include clear headings, key statistics, and actionable insights.
...
`;
    ...
    // Audit Logging
    console.log(`[Audit Log] Custom Report Generated`, {
