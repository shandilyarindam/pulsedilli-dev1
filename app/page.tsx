        ...
        <Card className="rounded-2xl border border-[#E8EDF5] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] bg-[#FFFFFF]">
          <CardContent className="p-2 flex flex-col justify-between h-full">
            <div className="flex items-center gap-3 mb-2">
              <div className="h-9 w-9 rounded-full bg-[#2563EB] flex items-center justify-center shrink-0 shadow-sm">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-800">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-slate-900 dark:text-white">Intervene: {selectedComplaint.id}</h2>
              </div>
              <button 
                onClick={() => setIsModalOpen(false)}
                className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition bg-white dark:bg-slate-800 rounded-full p-1 border border-slate-200 dark:border-slate-700"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-6">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-2">{selectedComplaint.summary}</h3>
              ...
              <textarea
                className="w-full h-28 p-3 bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500 resize-none text-slate-700 dark:text-slate-200"
            ...
            <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3">
              <button 
                onClick={() => setIsModalOpen(false)}
                className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 transition"
              >
                Cancel
              </button>
              <button 
                onClick={async () => {
                  ...
                  try {
                    await supabase.from('complaints').update({ notes: interventionComment }).eq('id', selectedComplaint.id);
